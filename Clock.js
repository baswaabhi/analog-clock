const hourE1=document.querySelector('.hour');
const minuteE1=document.querySelector('.minute');
const secondE1=document.querySelector('.second');

const timeE1=document.querySelector('.time');
const dateE1=document.querySelector('.date');
const toggleE1=document.querySelector('.toggle');

const days=['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
const months=['jan','feb','mar','apr','may','jun','july','aug','sep','oct','nov','dec'];

toggleE1.addEventListener('click',(e)=>{
    const html=document.querySelector('html')
    if(html.classList.contains('dark')){
        html.classList.remove('dark')
        e.target.innerHTML='Dark Mode'
    }else{
        html.classList.add('dark')
        e.target.innerHTML='Light Mode'
    }
})

function setTime(){
    const time=new Date();
    const month=time.getMonth();
    const day=time.getDay();
    const date=time.getDate();
    const hours=time.getHours();

    const hoursForClock=hours >=13 ? hours % 12 : hours;
     const minutes=time.getMinutes();
     const seconds=time.getSeconds();
     const ampm=hours >= 12 ? 'PM' :'AM'
     
     hourE1.style.transform=`translate(-50%,-100%)
     rotate(${scale(hoursForClock,0,11,0,359)}deg)`

     minuteE1.style.transform=`translate(-50%,-100%)
     rotate(${scale(minutes,0,59,0,359)}deg)`

     secondE1.style.transform=`translate(-50%,-100%)
     rotate(${scale(seconds,0,59,0,359)}deg)`

     timeE1.innerHTML=`${hoursForClock}:${minutes < 10 ? `0${minutes}`:minutes} ${ampm}`
     dateE1.innerHTML=`${days[day]},${months[month]}<span class='circle'>${date}</span>`
 
}
    const scale=(num, in_min, in_max, out_min, out_max) =>{
    return (num-in_min)*(out_max-out_min)/(in_max-in_min)+out_min;
    }
    setTime()
    setInterval(setTime,1000)

